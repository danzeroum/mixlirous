...
| QA-0005 | média | backend (test_correlacao xfail) + docs/03 §1 | `crates/audio_api` (middleware) | Contrato docs/03: "cliente envia `traceparent` (W3C); servidor devolve no response" — **não implementado** (extractor existe, eco não). Sem header de correlação em ambas as origens | corrigido | (este commit) | `cargo test -p audio_api --test qa_contracts` (10/10) + `cargo test -p audio_api --lib` (11 testes em `middleware::trace`) | laudos/qa-0005-antes-depois.txt (a gerar contra suíte) |
| QA-0006 | média | backend (test_erro_estruturado, skip) + docs/03 §1 | `crates/audio_api` (fallback de erro) | Contrato docs/03: erros `application/problem+json` (RFC 7807) — 404 de /api/* devolve corpo **vazio**; handlers devolvem texto puro | corrigido | (este commit) | `cargo test -p audio_api --test qa_contracts` (10/10) + `cargo test -p audio_api --lib` (6 testes em `problem`) | laudos/qa-0006-antes-depois.txt (a gerar contra suíte) |
