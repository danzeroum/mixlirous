//! Testes de invariantes pós-processamento (I1 clipping, I4 estalos).
//!
//! Estes testes usam `post_process`/`check_invariants` diretamente para
//! verificar as garantias de qualidade da saída do pipeline (invariantes I1 e
...
fn i1_sine_wave_no_clipping_after_post_process() {
    // Senoide de amplitude 0.9 já está dentro de ±1.0 — não deve gerar clipping.
    let sr = 44100u32;
...
fn i1_clipped_signal_is_limited() {
    ...
    // Cria duas regiões de clipping: 100 amostras a +2.0 e 100 a -3.0.
    for i in 0..100 {
        pcm[1000 + i] = 2.0;
        pcm[2000 + i] = -3.0;
    }
...
fn i4_smooth_signal_no_false_clicks() {
    // Senoide suave não deve ter estalos.
    let sr = 44100u32;
...
fn i4_impulse_is_detected_and_smoothed() {
    ...
    // Após suavização, nenhum salto deve exceder o limiar.
...
fn post_process_idempotent() {
    // Aplicar duas vezes não deve mudar nada na segunda.
    ...
    assert_eq!(pcm, snapshot, "segunda passagem não deveria mudar nada");
