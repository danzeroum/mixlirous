...
impl AudioRepo for InMemoryRepo {
    async fn save_job(&self, job_id: Uuid, tenant_id: Uuid, user_id: Uuid, config: &PipelineConfig, blocks: &[BeatBlock]) -> Result<(), RepoError> {
        ...
        let record = JobRecord {
            id: job_id, tenant_id, user_id,
            config: serde_json::to_value(config)?,
            blocks: serde_json::to_value(blocks)?,
            status: JobStatus::Queued,
            worker_id: None, attempts: 0, last_heartbeat: None,
    ...
    async fn get_job(&self, job_id: Uuid, tenant_id: Uuid) -> Result<JobRecord, RepoError> {
        ...
        state.jobs.get(&job_id).filter(|r| r.tenant_id == tenant_id).cloned().ok_or(RepoError::NotFound(job_id))
    }

    async fn list_jobs(&self, tenant_id: Uuid) -> Result<Vec<JobRecord>, RepoError> {
        let state = self.state.read().await;
        Ok(state.jobs.values().filter(|r| r.tenant_id == tenant_id).cloned().collect())
    }
    ...
    async fn transition_job(&self, job_id: Uuid, new_status: JobStatus, audit_action: &str) -> Result<(), RepoError> {
        let mut state = self.state.write().await;
        let now = Utc::now();
        let job = state.jobs.get_mut(&job_id).ok_or(RepoError::NotFound(job_id))?;
        job.status = new_status.clone();
        job.updated_at = now;
        state.audit.push(AuditRecord { job_id, action: audit_action.to_string(), new_status, occurred_at: now });
        Ok(())
    }

    async fn list_audit_records(&self, job_id: Uuid) -> Result<Vec<AuditRecord>, RepoError> {
        let state = self.state.read().await;
        Ok(state.audit.iter().filter(|r| r.job_id == job_id).cloned().collect())
    }
    ...
    async fn save_consent(&self, tenant_id: Uuid, provider: String) -> Result<ConsentRecord, RepoError> {
        let mut state = self.state.write().await;
        let record = ConsentRecord { tenant_id, assisted_mode_accepted_at: Utc::now(), provider_at_accept: provider };
    ...
    async fn claim_next_job(&self, worker_id: Uuid) -> Result<Option<JobRecord>, RepoError> {
        ...
        match job_id {
            Some(id) => {
                ...
                state.audit.push(AuditRecord { job_id: id, action: "JOB_CLAIMED".to_string(), new_status: JobStatus::Processing, occurred_at: now });
                Ok(Some(job_clone))
            }
            None => Ok(None),
        }
    ...
    async fn heartbeat(&self, job_id: Uuid, worker_id: Uuid) -> Result<(), RepoError> {
        ...
        let job = state.jobs.get_mut(&job_id).ok_or(RepoError::NotFound(job_id))?;
        match job.worker_id {
            Some(wid) if wid == worker_id => { job.last_heartbeat = Some(now); job.updated_at = now; Ok(()) }
    ...
    async fn fail_and_retry(&self, job_id: Uuid, max_attempts: u8) -> Result<(), RepoError> {
        ...
        if job.attempts >= max_attempts {
            ...
            state.audit.push(AuditRecord { job_id, action: "JOB_FAILED".to_string(), new_status: JobStatus::Failed, occurred_at: now });
        } else {
            job.status = JobStatus::Queued;
            state.audit.push(AuditRecord { job_id, action: "JOB_RETRY".to_string(), new_status: JobStatus::Queued, occurred_at: now });
        }
        Ok(())
    }
}
...
mod tests {
    ...
    async fn test_heartbeat_updates_last_heartbeat() {
        ...
        let hb_before = repo.get_job(job_id, tenant_id).await.unwrap().last_heartbeat.unwrap();
        tokio::time::sleep(std::time::Duration::from_millis(10)).await;
        repo.heartbeat(job_id, worker_id).await.unwrap();
        let hb_after = repo.get_job(job_id, tenant_id).await.unwrap().last_heartbeat.unwrap();
    ...
    async fn test_heartbeat_rejects_wrong_worker() {
        ...
        assert!(matches!(repo.heartbeat(job_id, worker2).await.unwrap_err(), RepoError::AlreadyClaimed(_)));
    }
    ...
    async fn test_concurrent_claims_each_job_once() {
        let repo = InMemoryRepo::new();
        let tenant_id = Uuid::new_v4();
        for _ in 0..10 { repo.save_job(Uuid::new_v4(), tenant_id, Uuid::new_v4(), &PipelineConfig::default(), &[]).await.unwrap(); }
        let mut handles = vec![];
        for _ in 0..10 { let repo_clone = repo.clone(); handles.push(tokio::spawn(async move { repo_clone.claim_next_job(Uuid::new_v4()).await })); }
        let mut claimed = 0;
