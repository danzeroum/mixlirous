...
pub async fn run_recovery(state: &AppState) -> Result<RecoveryReport, String> {
    ...
    let all_jobs = state.repo.list_jobs(uuid::Uuid::nil()).await
    ...
    state.hub.publish(uuid::Uuid::nil(), "recovery.report",
        serde_json::json!({"requeued": report.requeued})).await;
    Ok(report)
}
...
mod tests {
    ...
    fn test_state() -> AppState {
        let config = AppConfig {
            ...
            llm: LlmConfig { provider: "mock".to_string(), model: "mock".to_string(), base_url: "".to_string(), temperature: 0.7, max_tools: 5, timeout_sec: 30 },
            observability: ObservabilityConfig { otel_collector_endpoint: "".to_string(), prometheus_port: 9090, grafana_url: "".to_string() },
