...
function PrivacyPanel({ info, politica, consentAceitoEm, onAceitar, onRevogar, compacto }: Props) {
  ...
  return (
    <div
      ...
      className={`rounded-lg border border-surface-600 bg-surface-800 ${compacto ? 'p-3' : 'p-4'}`}
    >
      <h3 className={`font-bold text-ink-100 ${compacto ? 'text-sm' : 'text-base'}`}>
        IA, dados e privacidade
      </h3>

      <dl className={`mt-2 space-y-1 text-xs ${compacto ? '' : 'text-sm'}`}>
        <div className="flex gap-2">
          <dt className="text-ink-400 min-w-36">Provedor do assistente:</dt>
          <dd className="text-ink-100 font-medium">
            {info ? `${info.llm_provider} (${info.llm_model})` : 'carregando…'}
          </dd>
        </div>
        <div className="flex gap-2">
          <dt className="text-ink-400 min-w-36">O que vai para a IA:</dt>
          <dd className="text-ink-100" data-testid="privacy-egresso">
            {egressoTexto}
            {!audioNaoEnviado && (
        ...
        <div className="flex gap-2">
          <dt className="text-ink-400 min-w-36">Onde fica o áudio:</dt>
        ...
        <div className="flex gap-2">
          <dt className="text-ink-400 min-w-36">Ferramentas da IA:</dt>
          <dd className="text-ink-100">só usa ferramentas marcadas como disponíveis nesta instalação.</dd>
        </div>
      </dl>

      {aceitoEm ? (
        <div className="mt-3" data-testid="consent-status">
          <p className="text-xs text-action-300">
          ...
          <button
            ...
            className="mt-2 px-3 py-1.5 text-xs bg-danger-800/70 hover:bg-danger-700 text-danger-200 rounded border border-danger-800 disabled:opacity-50"
        ...
        <div className="mt-3">
          {revogadoLocal && (
            <p className="text-xs text-warn-300 mb-2" role="status" data-testid="consent-revoked">
          ...
          <button
            ...
            className="px-3 py-1.5 text-sm bg-ai-600 hover:bg-ai-500 text-white rounded disabled:opacity-50"
          >
